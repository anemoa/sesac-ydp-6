// 내장 메소드란?
// - 프로그래밍 언어나 프레임워크에서 기본적으로 제공되는 메소드
// - 따로 설치하거나 추가 설정할 필요가 없다

// 문자열 관련 내장 메소드
// - 문자열 다루는데 유용한 기능들을 제공
...
// 문자열의 길이 (length는 메소드가 아닌 속성이다)
console.log(str.length); // 14
console.log(str2.length); // 24
...
// 매개변수로 아무 값이 들어갖 않는다면 -1 반환


// slice() 문자열 자르기
// 음수값이 들어간다면 뒤에서부터 순서를 센다.
console.log(str.slice(10)); // hday - 10번 위치의 글자부터 
console.log(str.slice(1, 5)); // appy - start (1) ~ end-1(4) 까지 자르기
console.log(str.slice(-4)); // hday - 뒤에서 4번째 위치한 글자부터 끝까지
...
let arr = [1, 2, 3, 4, 5], arr2 =['늑대', '펭귄', '토끼', '호랑이'];
...
// console.log(arr);
// arr[8] = 100; // 인덱스를 건너뛰면 빈 값(empty)가 들어가게 됨
// console.log(arr);
...
// length: 배열의 길이 반환 (이것만 속성)
console.log(arr.length); // 6

// includes(요소): 요소가 있는지 없는지 검사. (true, false)
console.log(arr2.includes('토끼')); // true

// reverse()
// console.log(arr.reverse()); // 원본 배열이 변경됨
// console.log(arr);

// join(): join 안의 문자열 기준으로 병함
console.log(arr2);
