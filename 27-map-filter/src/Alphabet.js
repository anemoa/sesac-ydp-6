...
// 해석: deleteAlpha(value.id)라는 함수 호출을 나중에 실행되도록 준비해둔 "함수 참조" 이다.

// 나중이 실행: 컴포넌트가 렌더링될때 이 함수는 실행되지 않고, 이벤트가 발생할 때만 실행된다.
// value.id라는 인자가 이벤트가 발생할 때, deleteAlpha 함수에 전달되면서 실행한다.
// 필요성: 인자를 넘겨주는 함수 호출을 연기하기 위해 화살표 함수, 즉 콜백 함수 형태로 작성해야 한다.

// 3. deleteAlpha 함수 호출에서 인자 없이 사용하면?
// 해석: 그냥 deleteAlpha 라고만 작성할 수 있음
// 이 경우에도 함수는 참조로서 전달됨
// 인자를 넘겨줄 필요가 없으므로, 화살표 함수로 감쌀 필요도 없다.

// 요약
// deleteAlpha(value.id)는 즉시 실행을 의미
// () => deleteAlpha(value.id)는 이벤트 발생시 실행을 의미, 인자를 넘겨줄 때 사용
