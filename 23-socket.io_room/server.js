...
const io = socketIO(server); // sockt 서버
...
// 사용자 A( socket.id : '5678', userName: 'lu')
...
function getUserList(room){
    ...
    // room에는 접속한 룸 id
    // 특정 채팅방에 접속한 socekt.id 집합 값을 찾음
    ...
    // io.sockets.adapter
    // 소켓 서버 간의 상태를 관리하는 역할을 담당하는 객체 :: 소켓들으 연결 상태와 방(room) 정보를 관리

    // io.sockets.adapter.room
    // 모든 방(room) 정보를 저장하고 있는 객체 :: 각 방에 접속한 클라이언트들의 정보를 포함

    // get(room)
    // 위 객체에서 특정 방(room) ID에 해당하는 정보를 가져오는 역할 :: 해당 방에 접속한 클라이언트 들의 소켓

    // 방에 클라이언트가 있는 경우에 실행
    if(clients){
        // 각 socket.ID에 대한 반복 실행
    ...
    return users; // users = [{userName: "Ace", key: "1234"}]
}
...
io.on('connection', (socket) =>{
    ...
    socket.on('create', (res) => {
        ...
        socket.roomName = res.roomName; // 소켓 객체에 없는 메소드를 생성
        socket.userName = res.userName;
        console.log('socket.roomName >>', socket.roomName);

        // [3] 공지 알림 (본인 제외)
        // 나를 제외한 모든 방 사람들에게 메세지 전달
        socket.to(res.roomName).emit('notice', `${socket.userName} 님이 입장했어요`);
    ...
    socket.on('sendMessage', (res) => {
        console.log('sendMessage >>', res); // res: { message: '안녕', user: '둉둉', select: 'all' }
