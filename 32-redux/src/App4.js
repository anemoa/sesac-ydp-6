...
import { Box1Container, Box2Container, Box3Container, Box4Container } from './containers/BoxesContainers';
...
/*
    ...
    간단한 구조: 모든 컴포넌트가 동일한 파일에 정의되어 있었고, 리덕스 상태와 액션이 특정 컴포넌트에만 연결되어 있다.
    단일 책임 원칙 부족: 상태 관리와 UI 렌더링이 혼합되어 있으며, 상태와 액션을 처리하는 로직이 컴포넌트 내부에 포함되어 있다

    App4.js
    구조적 분리: 컨테이너 컴포넌트와 프레젠테이셔널 컴포넌트가 분리되어 있음
    단일 책임 원칙: Box4Container는 상태 관리와 액션 디스패치를 담당하고 Bo4는 UI 렌더링만 담당
