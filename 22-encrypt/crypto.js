...
// -> 평문(input)으로 돌아갈 수는 없지만 output이 동일하다면 input이 동일함까지는 유추 가능
...
const saltAndHashPw = (pw) =>{
    ...
    // pbkdf2Sync(비밀번호 원문, 솔트, 반복횟수, 키의 길이, 알고리즘)
    const hash = crypto.
        pbkdf2Sync(pw, salt, iterations, keylen, digest) // pw 값을 암호화
        .toString('base64'); // 암호화 된 Buffer 형식의 데이터를 base64 문자열로 변환해서 저장하거나 전송하기 쉽도록
...
console.log(`비밀번호가 ${isMatch ? '일치해요' : '일치하지 않는다'}`);