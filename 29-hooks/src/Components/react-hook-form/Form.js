...
// 언컨트롤드: 폼 필드가 업데이트 될 때마다 전체 돔이 리렌더링 되지 않도록 한다 ->  최적화 관련됨
// 필요할 때만 폼 필드의 상태를 업데이트 하여 불필요한 렌더링을 줄이고, 결과적으로 앱의 성능을 향상시킨다.

// 일반 폼 작성 vs RHF 라이브러리 차이
// 일반 폼
//  - 각 입력 필드의 상태를 useState로 관리해야 함
//  - 모든 입력값의 상태를 관리하고, 값이 바뀔때마다 전체 컴포넌트가 리렌더링 될 수 있음.
//  - 검증 로직을 직접 다 작성해야 함
//  - 폼 제출 시, 각 입력 필드의 상태들을 모아서 폼 데이터를 수집해야 함
//  - 간단한 폼 양식 작성할 땐 사용.

// RHF 라이브러리
//  - 코드의 간결화
//  - 비제어 컨포넌트 / 필요한 경우에만 리렌더링 된다.
//  - 다양한 검증 규칙을 쉽게 설정
//  - 'handleSubmit' 이거 하나로 모든 포머 데이터를 쉽게 관리
//  - 복잡하고 대규모 폼 양식을 작성할 때 사용

// Q) why? ... 을 사용하는가
// React 컴포넌트에서 ... 연산자는 객체나 배열을 펼쳐서 각각의 요소나 속성을 개별적인 prop으로 전달할 때 사용된다.
// 이 문법을 사용하면 코드가 간결해지고, 여러 개의 props를 한 번에 전달할 수 있다.

/** 
 * props?
 * - 컴포넌트가 전달되는 입력 값
 * - 부모 컴포넌특라 자식 컴포넌트에 데이터를 전달할 때 사용되며,
 * 이 props를 통해 컴포넌트의 동작이나 출력이 결정된다
*/

// React에서는 각 객체의 속성을 컴포넌트의 props로 전달할 때
// 각 속성을 개별적으로 전달할 수 있다.
...
// 직접 전달
{/* <MyCom name={person.name} color={person.color}/> */}

// 스프레드 연산자를 사용해서 전달
{/* <MyCom {...person} /> */}

/* 
    register 함수는 입력 필드를 폼 상태에 등록하고 제어하는데 필요한 여러 속성을 포함하는 객체를 반환한다.
    ...
    이 객체를 input 컴포넌트에 전달하기 위해서 스프레드 연산자를 사용하는 겁니다.
*/